...
import (
	"bytes"
	"github.com/iand/image"
	"github.com/iand/image/color"
	"github.com/iand/image/color/palette"
	_ "github.com/iand/image/png"
	"io/ioutil"
	"math/rand"
	"os"
	"reflect"
...
func TestSubImage(t *testing.T) {
	...
	if averageDelta(m0, m1) != 0 {
		t.Fatalf("github.com/iand/images differ")
	}
}
...
func testEncodeAll(t *testing.T, go1Dot5Fields bool, useGlobalColorModel bool) {
	...
	if len(g0.Image) != len(g1.Image) {
		t.Fatalf("github.com/iand/image lengths differ: %d and %d", len(g0.Image), len(g1.Image))
	}
	if len(g1.Image) != len(g1.Delay) {
		t.Fatalf("github.com/iand/image and delay lengths differ: %d and %d", len(g1.Image), len(g1.Delay))
	}
	if len(g1.Image) != len(g1.Disposal) {
		t.Fatalf("github.com/iand/image and disposal lengths differ: %d and %d", len(g1.Image), len(g1.Disposal))
	}
